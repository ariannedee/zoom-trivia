...
def player_answers(request, game_id, round_num):
    ...
    if _round.lightning:
        messages.add_message(
            request, messages.ERROR, "You cannot submit answers for lightning rounds"
        )
        return redirect("games:game", game_id)
    team_id = request.session.get("team_id")
    ...
    if team_id:
        context["answers"] = {
            answer.question.pk: answer.answer for answer in _round.get_team_answers(team_id)
        }
    else:
        messages.add_message(
            request,
    ...
    if request.method == "POST":
        ...
        for question in _round.questions.all():
            team_answer, created = TeamAnswer.objects.get_or_create(
                team_id=team_id, question=question
            )
            ...
            if not answer:
                messages.add_message(
                    request,
                    messages.WARNING,
                    f"No answer submitted for question {question.number}",
                )
            else:
